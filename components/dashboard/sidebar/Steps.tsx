...
const Steps = ({ title, checked, isToggle, className }: StepsProps) => {
  return (
    <li className="flex gap-x-3 items-center">
      <span
        className={twMerge(
          "relative w-5 h-5 rounded-full border border-[#FAFAFA] xl:w-8 xl:h-8 after:absolute after:w-[2px] after:h-[28px] after:bg-[#FAFAFA] after:-bottom-10 after:left-2 xl:after:left-3",
          className
        )}
      >
        {checked && (
      ...
      <span className={twMerge("hidden lg:block", isToggle ? 'block' : 'hidden')}>{title}</span>
    </li>
  );
};
