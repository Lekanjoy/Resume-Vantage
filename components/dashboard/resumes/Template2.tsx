...
const Template2 = ({ resumeData }: resumeDataProps) => {
  const fullName = resumeData.fname + " " + resumeData.lname;
  const location = resumeData.city + ", " + resumeData.country;

  return (
    <section className="bg-white h-[300px] max-w-[200px] overflow-y-auto scrollbar-hide p-1 flex flex-col lg:min-w-full">
      {/* Header Section */}
              ...
              <div key={exp._id}>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-blue-500 text-[6px]">{exp.company}</p>
                  </div>
        ...
        {resumeData?.education?.map((edu, index) => {
          return (
            <div key={edu._id}>
              ...
              <p className="text-[6px] text-gray-600">{edu.schoolLocation}</p>
